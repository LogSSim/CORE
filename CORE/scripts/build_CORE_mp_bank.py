#!/usr/bin/env python3
"""Build the shared terminal prototype for CORE_mp stage 2.

The script encodes each selected episode's last frame with the stage-1 terminal
encoder and saves their normalized average.
"""
...
def _load_policy(args):
    ...
    if not hasattr(policy, "term_module"):
        raise RuntimeError("Loaded policy has no term_module. Use CORE_mp stage1 checkpoint.")
    if missing:
        print(f"[build_CORE_mp_bank] missing keys: {missing}")
    if unexpected:
        print(f"[build_CORE_mp_bank] unexpected keys: {unexpected}")
...
def _save_bank(args, embeddings, episode_indices, metadata, checkpoint_path, zarr_path):
    ...
    print(f"[build_CORE_mp_bank] saved mean goal bank to {out_dir}")


def parse_args():
    parser = argparse.ArgumentParser(description="Build CORE-MP shared terminal prototype bank.")
    ...
    parser.add_argument("--config-name", default="CORE_mp")
    parser.add_argument("--k", type=int, default=1, help="Output folder suffix k_K; no KMeans is run.")
    parser.add_argument("--output-root", default="data/goal_bank_CORE_mp")
