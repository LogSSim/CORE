...
CONFIG_NAME="CORE_dp3"
TASK_NAME="metaworld_box-close"
GPU_ID="0"
SEED="0"
RUN_ROOT="${PROJECT_DIR}/data/outputs"
GOAL_BANK_ROOT="${PROJECT_DIR}/data/goal_bank_CORE_dp3"
STAGE1_NUM="20"
STAGE3_NUM="10"
STAGE1_EPOCHS="600"
TERMINAL_WINDOW="1"
...
case "${STAGE}" in
  stage1|1)
    # Stage1 only trains terminal_encoder; disable env rollout.
    python "${PROJECT_DIR}/train.py" \
      "${COMMON_OVERRIDES[@]}" \
      "terminal_goal.stage=1" \
      "terminal_goal.stage1_train_backbone=false" \
      "policy.stage1_train_backbone=false" \
  ...
  stage2|2)
    ...
    python "${PROJECT_DIR}/scripts/build_CORE_dp3_bank.py" \
