"""CORE policy with the DP3 backbone."""
...
from core.model.mean.conditional_unet1d import ConditionalUnet1D
...
class COREDP3BasePolicy(BasePolicy):
    def __init__(
        self,
        shape_meta: dict,
        noise_scheduler,
        ...
        **kwargs,
        ...
        if not obs_as_global_cond:
            raise ValueError("COREDP3Policy requires obs_as_global_cond=True.")
        ...
        self.obs_encoder = COREPointNetEncoder(
        ...
        cprint(
            f"[CORE-DP3-Base] terminal_aux={self.use_terminal_aux}, "
    ...
    def load_terminal_encoder_checkpoint(self, checkpoint_path):
        ...
        cprint(
            f"[CORE-DP3-Base] loaded terminal encoder {path}, "
...
class COREDP3Policy(COREDP3BasePolicy):
    """Public DP3-backed CORE policy used by the three-stage training script."""

    def __init__(self, *args, goal_cond_gate_init=0.0, stage1_train_backbone=False, **kwargs):
        self.stage1_train_backbone = bool(stage1_train_backbone)
        super().__init__(*args, **kwargs)
        self.goal_cond_norm = nn.LayerNorm(self.goal_feature_dim)
        self.goal_cond_gate = nn.Parameter(
            torch.tensor(float(goal_cond_gate_init), dtype=torch.float32)
        )
        if self.use_terminal_aux and not self.stage1_train_backbone:
            self._freeze_dp3_backbone()
        cprint(
            f"[CORE-DP3] goal_feature_dim={self.goal_feature_dim}, "
            f"goal_cond_gate_init={float(goal_cond_gate_init)}, "
            f"stage1_train_backbone={self.stage1_train_backbone}",
            "yellow",
        )

    def _freeze_dp3_backbone(self):
        for param in self.obs_encoder.parameters():
            param.requires_grad_(False)
        for param in self.model.parameters():
            param.requires_grad_(False)
        self.goal_cond_norm.requires_grad_(False)
        self.goal_cond_gate.requires_grad_(False)
    ...
    def _apply_goal_conditioning(self, global_cond, nobs):
        ...
        if global_cond.ndim != 2:
            raise ValueError(
                f"CORE goal conditioning expects [B, D] global_cond, got {global_cond.shape}"
            )
        current_pc = nobs["point_cloud"][:, self.n_obs_steps - 1, ...]
        if self.freeze_terminal_encoder:
            self.terminal_encoder.eval()
        ...
        z_goal = self._select_goal_prototype(z_curr)
        goal_feat = torch.cat([z_curr, z_goal, z_goal - z_curr], dim=-1)
        extra_cond = self.goal_cond_norm(goal_feat)
        extra_cond = self.goal_cond_gate * extra_cond
        return torch.cat([global_cond, extra_cond], dim=-1)

    def compute_loss(self, batch):
        if self.use_terminal_aux and not self.stage1_train_backbone:
            aux_loss, aux_metrics = self.compute_terminal_aux_loss(batch)
            loss_dict = {
                "base_loss": 0.0,
                "bc_loss": aux_loss.item(),
            }
