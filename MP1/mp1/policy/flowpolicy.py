...
class FlowPolicy(BasePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for i in range(sde.sample_N):
            ...
            pred = self.model(z, t*99, local_cond=local_cond, global_cond=global_cond, training=False) ### Copy from models/utils.py 
    ...
    def compute_loss(self, batch):
        ...
        + (~less_than_threshold) * x_at_segment_ends
            ...
            return res
        vt, feat_t = self.model(xt, t*99, cond=local_cond, global_cond=global_cond)
        vr, _ = self.model(xr, r*99, local_cond=local_cond, global_cond=global_cond)
        # mask
        ...
        for i in range(feat_t.shape[0]):
            dis_loss += self.dispersive_loss(feat_t)
