...
class Meanpolicy(BasePolicy):
    def __init__(self, 
        ...
        if self.use_short_term_memory and self.n_obs_steps < self.num_memory_frames + 1:
            raise ValueError(
                "short_term_memory requires n_obs_steps >= num_memory_frames + 1, "
                f"got n_obs_steps={self.n_obs_steps}, num_memory_frames={self.num_memory_frames}"
            )
