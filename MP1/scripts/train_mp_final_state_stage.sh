...
usage() {
  cat <<'EOF'
Usage:
  bash MP1/scripts/train_mp_final_state_stage.sh [options] stage1 [hydra_overrides...]
  bash MP1/scripts/train_mp_final_state_stage.sh [options] stage2 [stage1_run_dir_or_ckpt] [stage2_args...]
  bash MP1/scripts/train_mp_final_state_stage.sh [options] stage3 [stage2_feature.npz] [hydra_overrides...]

Stages:
  stage1    Train Meanpolicy on the first 20 zarr demos.
  stage2    Encode the final frame of the first 20 zarr demos and cluster features.
  stage3    Train Meanpolicy on the first 10 zarr demos with final-state conditions.

Options:
  -c, --config NAME         Hydra config name. Default: mp_final_state_stage
...
CONFIG_NAME="${CONFIG_NAME:-mp_final_state_stage}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ckpt|--stage1-ckpt) STAGE1_CKPT="$2"; shift 2 ;;
    --seed) SEED="$2"; shift 2 ;;
...
if [[ -z "${FEATURE_PATH}" ]]; then
  FEATURE_PATH="${PROJECT_DIR}/data/final_state_features/${TASK_NAME}_mp_stage2_final_state_k${NUM_CLUSTERS}.npz"
fi
DEVICE="cuda:${GPU}"
...
take_checkpoint_arg() {
  ...
  STAGE1_CKPT_RESOLVED="$(resolve_checkpoint_path "${ckpt}")"
  REMAINING_ARGS=("$@")
}
