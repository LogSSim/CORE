#!/usr/bin/env python3
"""Build goal prototype banks for MeanpolicyTerminalGoalV2 stage 2."""
...
def _load_policy(args):
    ...
    if not hasattr(policy, "term_module"):
        raise RuntimeError("Loaded policy has no term_module. Use mp_terminal_goal_v2_stage stage1 checkpoint.")
    if missing:
        print(f"[build_mp_terminal_goal_v2_bank] missing keys: {missing}")
    if unexpected:
        print(f"[build_mp_terminal_goal_v2_bank] unexpected keys: {unexpected}")
...
def _save_banks(args, embeddings, medoid_point_clouds, episode_indices, metadata, checkpoint_path, zarr_path):
    ...
    for k in args.ks:
        ...
        print(f"[build_mp_terminal_goal_v2_bank] saved K={int(k)} goal bank to {out_dir}")


def parse_args():
    parser = argparse.ArgumentParser(description="Build MP terminal-goal v2 prototype bank.")
    ...
    parser.add_argument("--config-name", default="mp_terminal_goal_v2_stage")
    parser.add_argument("--ks", nargs="+", type=int, default=[4])
    parser.add_argument("--terminal-window", type=int, default=8)
    parser.add_argument("--output-root", default="/data1/sjy/MP1/MP1/data/goal_bank_mp_v2")
