#!/usr/bin/env python3
"""Build goal prototype banks for DP3TerminalGoalV2Policy stage 2."""
...
def _load_policy(args):
    ...
    if not hasattr(policy, "terminal_encoder") or policy.terminal_encoder is None:
        raise RuntimeError("Loaded policy has no terminal_encoder. Use dp3_terminal_goal_v2_stage stage1 checkpoint.")
    if missing:
        print(f"[build_dp3_terminal_goal_v2_bank] missing keys: {missing}")
    if unexpected:
        print(f"[build_dp3_terminal_goal_v2_bank] unexpected keys: {unexpected}")
...
def _save_banks(args, embeddings, medoid_point_clouds, episode_indices, metadata, checkpoint_path, zarr_path):
    ...
    common_from_all = _normalized_mean(embeddings)
    ...
    for k in args.ks:
        ...
        cluster_sizes = np.bincount(labels, minlength=int(k)).astype(np.float32)
        weighted_common = (centers * cluster_sizes[:, None]).sum(axis=0)
        weighted_common = weighted_common / np.clip(np.linalg.norm(weighted_common), 1e-8, None)
        ...
        prototype_path = out_dir / "prototypes.npy"
        common_prototype_path = out_dir / "common_prototype.npy"
        np.save(prototype_path, centers)
        np.save(common_prototype_path, weighted_common.astype(np.float32))
        np.save(out_dir / "common_prototype_from_all.npy", common_from_all)
        np.save(out_dir / "cluster_sizes.npy", cluster_sizes.astype(np.int64))
            ...
            json.dump(
                {
                    ...
                    "common_prototype_path": str(common_prototype_path),
                    "common_prototype_from_all_path": str(out_dir / "common_prototype_from_all.npy"),
        ...
        print(
            "[build_dp3_terminal_goal_v2_bank] saved "
            f"K={int(k)} prototypes and common prototype to {out_dir}"
        )


def parse_args():
    parser = argparse.ArgumentParser(description="Build DP3 terminal-goal v2 prototype bank.")
    ...
    parser.add_argument("--config-name", default="dp3_terminal_goal_v2_stage")
    parser.add_argument("--ks", nargs="+", type=int, default=[4])
    parser.add_argument("--terminal-window", type=int, default=8)
    parser.add_argument("--output-root", default="/data1/sjy/MP1/MP1/data/goal_bank_v2")
