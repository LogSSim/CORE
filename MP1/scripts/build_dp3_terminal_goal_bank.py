#!/usr/bin/env python3
"""Build goal prototype banks for DP3TerminalGoalPolicy stage 2."""
...
def _load_policy(args):
    ...
    if not hasattr(policy, "terminal_encoder") or policy.terminal_encoder is None:
        raise RuntimeError("Loaded policy has no terminal_encoder. Use dp3_terminal_goal_stage stage1 checkpoint.")
    if missing:
        print(f"[build_dp3_terminal_goal_bank] missing keys: {missing}")
    if unexpected:
        print(f"[build_dp3_terminal_goal_bank] unexpected keys: {unexpected}")
...
def _save_banks(args, embeddings, medoid_point_clouds, episode_indices, metadata, checkpoint_path, zarr_path):
    ...
    for k in args.ks:
        ...
        print(f"[build_dp3_terminal_goal_bank] saved K={int(k)} goal bank to {out_dir}")


def parse_args():
    parser = argparse.ArgumentParser(description="Build DP3 terminal-goal prototype bank.")
    ...
    parser.add_argument("--config-name", default="dp3_terminal_goal_stage")
    parser.add_argument("--ks", nargs="+", type=int, default=[4])
    parser.add_argument("--terminal-window", type=int, default=8)
    parser.add_argument("--output-root", default="/data1/sjy/MP1/MP1/data/goal_bank")
